    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-[#111827] border border-white/8 rounded-2xl p-5 space-y-4">
        ...
        <input
          ...
          placeholder="Course link (Google Drive / YouTube playlist URL)"
          className="w-full bg-[#0B1120] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-[#00D4AA]"
        />
              ...
              <div key={c.id} className="bg-[#111827] border border-white/8 rounded-xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-500 text-xs">
                    {c.lessonCount} lessons · {c.priceNaira === 0 ? 'Free' : `₦${c.priceNaira.toLocaleString()}`} · {c.level}
                  </p>
                  {c.contentUrl ? (
                    <button
                      onClick={() => openLink(c.contentUrl as string)}
                      className="inline-flex items-center gap-1 text-[#00D4AA] text-xs mt-1 hover:underline"
                    >
                      <ExternalLink size={11} /> View course link
                    </button>
                  ) : (
                    <p className="text-gray-600 text-xs mt-1">No course link added</p>
                  )}
                </div>
                <button
                  onClick={() => openCourse(c.id)}
                  className="text-[#00D4AA] text-xs px-3 py-1.5 rounded-lg bg-[#00D4AA]/10 hover:bg-[#00D4AA]/20 transition-colors"
                >
                  Manage Lessons
                </button>
        ...
        <div className="bg-[#111827] border border-[#00D4AA]/30 rounded-2xl p-5 space-y-4">
            ...
            <div className="space-y-2">
              {selected.lessons.map((l: Lesson, i: number) => (
                <div key={l.id} className="flex items-center gap-3 bg-[#0B1120] rounded-xl p-3">
                  ...
                  {l.videoUrl ? (
                    <Video size={15} className="text-[#00D4AA]" />
                  ) : (
          ...
          <div className="border-t border-white/5 pt-4 space-y-3">
            ...
            <input
              ...
              placeholder="Video link (YouTube URL) — optional"
              className="w-full bg-[#0B1120] border border-white/10 rounded-xl px-4 py-2.5 text-white placeholder-gray-600 focus:outline-none focus:border-[#00D4AA] text-sm"
            />
            <textarea
              value={lContent}
          ...
          <button onClick={() => setSelected(null)} className="text-gray-500 text-xs hover:text-white">
            Close
          </button>
        </div>
      ) : null}
